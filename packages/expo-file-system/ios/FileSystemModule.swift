...
public final class FileSystemModule: Module {
  ...
  public func definition() -> ModuleDefinition {
    ...
    // swiftlint:disable:next closure_body_length
    AsyncFunction("uploadFileAsync") { (url: URL, from: FileSystemFile, options: UploadOptions?, promise: Promise) in
      try from.validatePermission(.read)
      guard from.exists else {
        throw UnableToUploadException("file does not exist: \(from.url.absoluteString)")
      }

      let uploadOptions = options ?? UploadOptions()

      var request = URLRequest(url: url)
      request.httpMethod = uploadOptions.httpMethod.rawValue

      for (key, value) in uploadOptions.headers {
        request.addValue(value, forHTTPHeaderField: key)
      }

      let fileData = try Data(contentsOf: from.url)

      switch uploadOptions.uploadType {
      case .binaryContent:
        if let mimeType = uploadOptions.mimeType {
      ...
      case .multipart:
        ...
        if let parameters = uploadOptions.parameters {
        ...
        let fieldName = uploadOptions.fieldName ?? "file"
        let fileName = from.url.lastPathComponent
        let mimeType = uploadOptions.mimeType ?? from.type ?? "application/octet-stream"
      ...
      let task = URLSession.shared.dataTask(with: request) { data, response, error in
        ...
        guard let httpResponse = response as? HTTPURLResponse else {
          promise.reject(UnableToUploadException("no response"))
          return
        }
